...
func getIntcode(input []int) []int {
	...
	for i < len(input) {
		...
		case 3:
			...
			if err != nil {
				log.Fatal("Error on user input.", err.Error())
			}

			targetIndex := input[i+1]
			input[targetIndex] = userInput
		...
		case 5:
			...
			if val1 != 0 {
				...
				if mode2 == 0 {
					newInstructionPointer = input[index2]
				}
		...
		case 8:
			...
			i += 4
		default:
			fmt.Println("Error. Got opcode ", opcode, "on instruction ", i)
			return input
		}
	}
	return input
